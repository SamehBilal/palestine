...
class StateController extends Controller
{
    ...
    protected $rname    = 'states';
    protected $bulk_can = 'states_bulk';
    protected $phrases  = [
        'index'     => 'States',
        'trash'     => 'Trash',
        'create'    => 'Add State',
    ...
    public function update(UpdateStateRequest $request, State $state)
    {
        ...
        $model = $state;
        $this->validate($request, $this->model::rules($update = true, $state->id));
    ...
    public function destroy(State $state)
    {
        ...
        $action = 'delete';
        if (empty($state)) {
            $state = $state->withTrashed()->firstOrFail();
            $action = Gate::allows('tags_fdelete') ? 'forceDelete' : '';
        }
        $state?->$action();
        $this->cleanCache($state);
        return back();
    }
